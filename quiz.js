...
function showQuestion() {
    ...
    questionTextEl.innerText = currentQuiz.question; // innerText to correctly display newlines
...
function selectChoice(button, selectedChoice, correctAnswer) {
    ...
    if (selectedChoice === correctAnswer) {
        ...
        button.disabled = false; // Allow re-trying the same button if needed, but better to allow others
        allChoiceBtns.forEach(btn => {
            if(btn.textContent !== correctAnswer) {
                 btn.disabled = false; // Re-enable incorrect options
            }
        });
        button.disabled = true; // Disable the specific wrong choice just clicked
    }
}
